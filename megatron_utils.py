...
def _nested_tensor_nbytes(obj: Any) -> int:
    if _is_tensor(obj):
        return _tensor_nbytes(obj)
    if isinstance(obj, dict):
        return sum(_nested_tensor_nbytes(v) for v in obj.values())
    if isinstance(obj, (list, tuple, set)):
        return sum(_nested_tensor_nbytes(v) for v in obj)
    return 0


def _pin_if_possible(t: torch.Tensor, pin_memory: bool) -> torch.Tensor:
    if not pin_memory:
        return t
...
def _to_cpu_tensor(t: torch.Tensor, pin_memory: bool) -> torch.Tensor:
    out = t.detach().to("cpu")
    return _pin_if_possible(out, pin_memory=pin_memory)
...
def _infer_optimizer_device(optimizer: torch.optim.Optimizer) -> torch.device:
...
def _resolve_tensor_by_name(mapping: dict[str, torch.Tensor], name: str) -> Optional[torch.Tensor]:
    return mapping.get(name, None)


def _move_container_tensors_inplace(
    obj: Any,
    *,
    to_cpu: bool,
    device: torch.device | str,
    ...
    if _is_tensor(obj):
        if to_cpu:
            return _to_cpu_tensor(obj, pin_memory=pin_memory)
        return _to_device_tensor(obj, device=device, non_blocking=non_blocking)

    if isinstance(obj, dict):
        for k, v in list(obj.items()):
            obj[k] = _move_container_tensors_inplace(
                v,
                to_cpu=to_cpu,
                device=device,
    ...
    if isinstance(obj, list):
        for i, v in enumerate(obj):
            obj[i] = _move_container_tensors_inplace(
                v,
                to_cpu=to_cpu,
                device=device,
                pin_memory=pin_memory,
    ...
    if isinstance(obj, tuple):
        return tuple(
            _move_container_tensors_inplace(
                v,
                to_cpu=to_cpu,
                device=device,
                pin_memory=pin_memory,
...
@dataclass
class ModuleParkingHandle:
    param_names: list[str] = field(default_factory=list)
    grad_names: list[str] = field(default_factory=list)
    buffer_names: list[str] = field(default_factory=list)

    param_devices: dict[str, torch.device] = field(default_factory=dict)
    grad_devices: dict[str, torch.device] = field(default_factory=dict)
    buffer_devices: dict[str, torch.device] = field(default_factory=dict)

    bytes_moved: int = 0


@dataclass
class OptimizerParkingHandle:
...
@dataclass
class TrainingParkingHandle:
    module_handles: dict[str, ModuleParkingHandle] = field(default_factory=dict)
    optimizer_handle: Optional[OptimizerParkingHandle] = None
...
def park_module_to_cpu(
    ...
    *,
    ...
    if only_frozen and only_trainable:
        raise ValueError("only_frozen and only_trainable cannot both be True")

    handle = ModuleParkingHandle()
    ...
    for name, p in module.named_parameters(recurse=True):
        ...
        handle.param_names.append(name)
        handle.param_devices[name] = p.device
        handle.bytes_moved += _tensor_nbytes(p.data)
        p.data = _to_cpu_tensor(p.data, pin_memory=pin_memory)

        if include_grads and p.grad is not None:
            handle.grad_names.append(name)
            handle.grad_devices[name] = p.grad.device
            handle.bytes_moved += _tensor_nbytes(p.grad)
            p.grad = _to_cpu_tensor(p.grad, pin_memory=pin_memory)

    if include_buffers:
        for name, b in module.named_buffers(recurse=True):
            if b is None:
                continue
            handle.buffer_names.append(name)
            handle.buffer_devices[name] = b.device
            handle.bytes_moved += _tensor_nbytes(b)
            b.data = _to_cpu_tensor(b.data, pin_memory=pin_memory)

    return handle


@torch.no_grad()
def restore_module_from_cpu(
    module: nn.Module,
    handle: ModuleParkingHandle,
    *,
    ...
    named_buffers = dict(module.named_buffers(recurse=True))

    for name in handle.param_names:
        p = _resolve_tensor_by_name(named_params, name)
        if p is None:
            continue
        target_device = device if device is not None else handle.param_devices[name]
        p.data = _to_device_tensor(p.data, target_device, non_blocking=non_blocking)

    for name in handle.grad_names:
        p = _resolve_tensor_by_name(named_params, name)
        if p is None or p.grad is None:
            continue
        target_device = device if device is not None else handle.grad_devices[name]
        p.grad = _to_device_tensor(p.grad, target_device, non_blocking=non_blocking)

    for name in handle.buffer_names:
        b = _resolve_tensor_by_name(named_buffers, name)
        if b is None:
            continue
        target_device = device if device is not None else handle.buffer_devices[name]
        b.data = _to_device_tensor(b.data, target_device, non_blocking=non_blocking)
...
def parked_module(
    ...
    *,
    ...
    only_trainable: bool = False,
    pin_memory: bool = True,
    restore_device: torch.device | str | None = None,
    non_blocking: bool = True,
):
    handle = park_module_to_cpu(
        module,
...
# ---------------------------------------------------------------------------
# Optimizer parking / restore
# ---------------------------------------------------------------------------

_COMMON_MASTER_PARAM_ATTRS = (
...
def optimizer_state_nbytes(optimizer: torch.optim.Optimizer) -> int:
    ...
    for state in optimizer.state.values():
        total += _nested_tensor_nbytes(state)
    return total


@torch.no_grad()
def park_optimizer_state_to_cpu(
    optimizer: torch.optim.Optimizer,
    *,
    pin_memory: bool = True,
    include_master_params: bool = True,
) -> OptimizerParkingHandle:
    original_device = _infer_optimizer_device(optimizer)
    handle = OptimizerParkingHandle(original_device=original_device)

    for state in optimizer.state.values():
        handle.bytes_moved += _nested_tensor_nbytes(state)
        _move_container_tensors_inplace(
            state,
            to_cpu=True,
            device=CPU_DEVICE,
    ...
    if include_master_params:
        for attr_name in _COMMON_MASTER_PARAM_ATTRS:
            if hasattr(optimizer, attr_name):
                attr_val = getattr(optimizer, attr_name)
                handle.bytes_moved += _nested_tensor_nbytes(attr_val)
                new_val = _move_container_tensors_inplace(
                    attr_val,
                    to_cpu=True,
                    device=CPU_DEVICE,
                    pin_memory=pin_memory,
                ...
                setattr(optimizer, attr_name, new_val)
                handle.extra_attr_names.append(attr_name)

    return handle


@torch.no_grad()
def restore_optimizer_state_from_cpu(
    optimizer: torch.optim.Optimizer,
    handle: OptimizerParkingHandle | None = None,
    *,
    device: torch.device | str | None = None,
    non_blocking: bool = True,
    include_master_params: bool = True,
    ...
    target_device = device if device is not None else (
        handle.original_device if handle is not None else _infer_optimizer_device(optimizer)
    )
    ...
    if include_master_params:
        ...
        if handle is not None:
            attr_names = handle.extra_attr_names
        else:
            attr_names = [name for name in _COMMON_MASTER_PARAM_ATTRS if hasattr(optimizer, name)]

        for attr_name in attr_names:
            if not hasattr(optimizer, attr_name):
                continue
            attr_val = getattr(optimizer, attr_name)
            new_val = _move_container_tensors_inplace(
                attr_val,
...
def parked_optimizer_state(
    ...
    *,
    ...
    handle = park_optimizer_state_to_cpu(
        optimizer,
        pin_memory=pin_memory,
    ...
    try:
        ...
        restore_optimizer_state_from_cpu(
            optimizer,
            handle,
            device=restore_device,
            non_blocking=non_blocking,
...
# ---------------------------------------------------------------------------
# Grouped training-state parking
# ---------------------------------------------------------------------------

@torch.no_grad()
def park_training_state(
    *,
    modules: Optional[dict[str, nn.Module]] = None,
    optimizer: Optional[torch.optim.Optimizer] = None,
    pin_memory: bool = True,
    include_optimizer_master_params: bool = True,
    include_module_buffers: bool = True,
    include_module_grads: bool = False,
    only_frozen_modules: bool = False,
) -> TrainingParkingHandle:
    handle = TrainingParkingHandle()

    if modules:
        for name, module in modules.items():
            handle.module_handles[name] = park_module_to_cpu(
                module,
                include_buffers=include_module_buffers,
                include_grads=include_module_grads,
                only_frozen=only_frozen_modules,
                pin_memory=pin_memory,
            )

    if optimizer is not None:
        handle.optimizer_handle = park_optimizer_state_to_cpu(
            optimizer,
            pin_memory=pin_memory,
            include_master_params=include_optimizer_master_params,
        )
...
def restore_training_state(
    ...
    *,
    ...
    if modules:
        for name, module in modules.items():
            if name in handle.module_handles:
                restore_module_from_cpu(
                    module,
    ...
    if optimizer is not None and handle.optimizer_handle is not None:
        restore_optimizer_state_from_cpu(
            optimizer,
            handle.optimizer_handle,
            device=device,
            non_blocking=non_blocking,
            include_master_params=include_optimizer_master_params,
        )
...
# ---------------------------------------------------------------------------
# Activation offloading, borrowed from trl
# ---------------------------------------------------------------------------
        ...
        def pack_tensor(activation: torch.Tensor) -> int:
            ...
            with stream_ctx:
                ...
                if has_broadcast:
                    ...
                    # Copy the full underlying storage, not just prod(shape) * itemsize bytes.
                    # The saved strides may reference storage beyond what `empty_like` would
                    # allocate (e.g. when `activation` is a narrow/slice of a wider tensor).
                    storage_numel = activation.untyped_storage().size() // activation.element_size()
                    cpu_storage_tensor = torch.empty(
                        storage_numel,
                        dtype=activation.dtype,
                        pin_memory=self.use_pin_memory,
                        device="cpu",
                    )
                    # View the GPU storage as a flat 1D tensor and copy it wholesale.
                    gpu_flat = torch.as_strided(
                        activation,
                        size=(storage_numel,),
                        stride=(1,),
                        storage_offset=0,
                    )
                    cpu_storage_tensor.copy_(gpu_flat, non_blocking=True)
                    cpu_tensor = cpu_storage_tensor
...
def memory_report(
    *,
    ...
    optimizer: Optional[torch.optim.Optimizer] = None,
    inactive_modules: Optional[dict[str, nn.Module]] = None,
) -> dict[str, float]:
    out: dict[str, float] = {}

    if model is not None:
        out["model_mib"] = module_nbytes(model) / (1024 ** 2)
        out["model_params_only_mib"] = module_nbytes(
            model,
            include_grad=False,
    ...
    if optimizer is not None:
        out["optimizer_state_mib"] = optimizer_state_nbytes(optimizer) / (1024 ** 2)

    if inactive_modules:
        for name, module in inactive_modules.items():
            out[f"{name}_mib"] = module_nbytes(module) / (1024 ** 2)
