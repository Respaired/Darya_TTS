...
def get_num_params(model: nn.Module) -> str:
...
def sample_masked_span(
    ...
    frac_max: float = 1.0,
    min_mask_len: int = 1,
    keep_at_least_one_unmasked: bool = False,
) -> torch.Tensor:
...
def extract_disc_features(
    ...
    disc_t: Optional[torch.Tensor] = None,
    disc_noise: Optional[torch.Tensor] = None,
):
...
def parse_args():
    ...
    dataset_group.add_argument("--datasets", type=str,
                               help="Path to JSON file containing a top-level datasets list")
    ...
    parser.add_argument("--use_speaker", action="store_true",
                        help="TitaNet speaker-conditioned training (requires --stage2)")
